...
MY_GUILD = discord.Object(id=957056959901732934)
...
async def on_message(message):
    ...
    if "sword" in message.content.lower():
        await message.channel.send(':fencer:')
...
@client.tree.command(name="add_tick", description="Add tick to list")
async def add_tick(interaction: discord.Interaction, tick: str, user: discord.User):
    if user is None:
